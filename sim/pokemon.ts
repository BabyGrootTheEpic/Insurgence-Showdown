...
export class Pokemon {
	...
	*/
	...
	transformInto(pokemon: Pokemon, effect?: Effect) {
		...
		for (const moveSlot of pokemon.moveSlots) {
			...
			this.moveSlots.push({
				move: moveName,
				id: moveSlot.id,
				pp: moveSlot.maxpp < 5 ? moveSlot.maxpp : 5,
						...
						if (numberOfMoves > 0) {
							...
							for (let i = 0; i < numberOfMoves; i++) {
								...
								this.moveSlots.push({
									move: slotMove.name,
									id: slotMove.id,
									pp: slotPP < 5 ? slotPP : 5,
									maxpp: slotPP < 5 ? slotPP : 5,
	...
	setType(newType: string | string[], enforce = false) {
		...
		if (!enforce) {
			if (this.hasAbility('multitype') || this.hasAbility('rkssystem')) {
				return false;
			}
		}
	...
	isGrounded(negateImmunity = false) {
		...
		if (this.hasAbility('omnitype') && !this.battle.suppressingAbility()) return false;
