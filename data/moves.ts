...
export const Moves: {[moveid: string]: MoveData} = {
	...
	angelwings: {
		...
		priority: 0,
		onTryHit(pokemon) {
			if (pokemon.volatiles['angelwings']) return false;
		},
		flags: {snatch: 1},
		volatileStatus: 'angelwings',
		condition: {
			onStart(pokemon) {
				this.add('-start', pokemon, 'Angel Wings');
			},
			//User becomes weak to all types, implemented in sim/battle-actions.ts
		},
		boosts: {
			atk: 2,
			spa: 2,
	...
	flashfreeze: {
		...
		onHit(target) {
			if (target.getTypes().join() === 'Ice' || !target.setType('Ice')) {
				// Flash Freeze should animate even when it fails.
	...
	beakblast: {
		...
		pp: 15,
		priority: -3,
		flags: {bullet: 1, protect: 1, distance: 1},
	...
	floatyfall: {
		...
		pp: 15,
		priority: 0,
		flags: {contact: 1, protect: 1, gravity: 1, distance: 1},
		secondary: {
			chance: 30,
			volatileStatus: 'flinch',
		},
		...
		condition: {
			...
			onSwitchIn(pokemon) {
				...
				if (pokemon.hasType('Electric') || pokemon.hasType('Ground')) {
					...
					4 * this.effectState.layers >= rand) {
					pokemon.trySetStatus('par', pokemon.side.foe.active[0]);
				} else if (2 * this.effectState.layers >= rand) {
					pokemon.trySetStatus('par', pokemon.side.foe.active[0]);
				}
			},
		},
		...
		condition: {
			...
			onSwitchIn(pokemon) {
				...
				if (pokemon.hasType('Ice') || pokemon.hasType('Fire')) {
					...
					2 * this.effectState.layers >= rand) {
					pokemon.trySetStatus('frz', pokemon.side.foe.active[0]);
				} else if (this.effectState.layers >= rand) {
					pokemon.trySetStatus('frz', pokemon.side.foe.active[0]);
				}
			},
		},
	...
	smackdown: {
		...
		condition: {
			...
			onStart(pokemon) {
				let applies = false;
				if (pokemon.hasType('Flying') || pokemon.hasAbility('levitate') || pokemon.hasAbility('omnitype')) applies = true;
	...
	terrainpulse: {
		...
		pp: 10,
		priority: 0,
		flags: {protect: 1, mirror: 1, pulse: 1, distance: 1},
		onModifyType(move, pokemon) {
			if (!pokemon.isGrounded()) return;
