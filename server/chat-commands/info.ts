...
export const commands: Chat.ChatCommands = {
	addedfeatures(target, room, user) {
		...
		this.sendReplyBox(
			`<p><strong>Formats:</strong> All of this server's formats are National Dex formats. Additionally they will allow MissingNo, Cosplay Pikachu, Let's GO Starters (even both on the same team), Spiky-eared Pichu, Eternal Flower Floette, Totems, multiple fusions on the same team, and Gems. The maximum Pok&eacute;mon level has been increased to 120. Use <code>/formatshelp</code> for details on specific formats.</p>` +
			`<p><strong>Added Pok&eacute;mon:</strong></p>` +
			...
			`<li>Zeta &amp; Omicron: <a href="https://pokemonzetaomicron.fandom.com/wiki/Starter_Pok%C3%A9mon#Stat_Modifications">Starters</a>, <a href="https://pokemonzetaomicron.fandom.com/wiki/Special_Pokemon#Obtainable_Delta_Pok.C3.A9mon">Delta Pok&eacute;mon</a>, <a href="https://pokemonzetaomicron.fandom.com/wiki/Fakemon">Fakemon</a>, <a href="https://pokemonzetaomicron.fandom.com/wiki/Strange_Machine">Strange Machine Mewtwo</a>, Mega Dugtrio, Mega Golurk, &amp; an additional Mega Evolution for Sceptile and Swampert.</li></ul>` +
			...
			`<p><strong>Added Abilities:</strong> All new abilities from <a href="https://wiki.p-insurgence.com/Category:Abilities">Insurgence</a> have been added.</p>` +
	...
	coverage(target, room, user) {
		...
		if (!dispTable) {
			...
			for (const type of dex.types.names()) {
				icon[type] = `<img src="https://raw.githubusercontent.com/BabyGrootTheEpic/Sprites/main/play.pokemonshowdown.com/sprites/types/${type.replace(/\?/g, '%3f')}.png" width="32" height="14">`;
				// row of icons at top
