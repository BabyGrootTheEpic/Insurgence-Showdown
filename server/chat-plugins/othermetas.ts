	...
	stone(target) {
		...
		for (const aStone of toDisplay) {
			...
			if (deltas.type && deltas.type !== 'mono') {
				buf += `<span class="col typecol"><img src="https://raw.githubusercontent.com/BabyGrootTheEpic/Sprites/main/play.pokemonshowdown.com/sprites/types/${deltas.type.replace(/\?/g, 'Q')}.png" alt="${deltas.type}" height="14" width="32"></span> `;
			} else {
